...
public class SerializableDictionaryPropertyDrawer : PropertyDrawer
{
	...
	static float DrawKeyValueLineExpand(SerializedProperty keyProperty, SerializedProperty valueProperty, Rect linePosition)
	{
		...
		SerializedProperty displayedValueProperty;
		bool displaySingleChild = TryGetSingleVisibleChild(valueProperty, out displayedValueProperty);
		if(!displaySingleChild)
			displayedValueProperty = valueProperty;

		float valuePropertyHeight = EditorGUI.GetPropertyHeight(displayedValueProperty, true);
		var valuePosition = linePosition;
		valuePosition.height = valuePropertyHeight;
		valuePosition.xMin += labelWidth;
		EditorGUI.indentLevel--;
		EditorGUI.PropertyField(
			...
			displaySingleChild ? TempContent(displayedValueProperty.displayName) : GUIContent.none,
			true);
		EditorGUI.indentLevel++;
	...
	static bool TryGetSingleVisibleChild(SerializedProperty property, out SerializedProperty childProperty)
	{
		childProperty = null;
		...
		if(!firstChildProperty.NextVisible(true) || SerializedProperty.EqualContents(firstChildProperty, endProperty))
			return false;

		var nextProperty = firstChildProperty.Copy();
		if(nextProperty.NextVisible(false) && !SerializedProperty.EqualContents(nextProperty, endProperty))
			return false;

		childProperty = firstChildProperty;
		return true;
	}

	static float GetDisplayedPropertyHeight(SerializedProperty property)
	{
		SerializedProperty childProperty;
		return TryGetSingleVisibleChild(property, out childProperty)
	...
	static void SaveProperty(SerializedProperty keyProperty, SerializedProperty valueProperty, int index, int otherIndex, ConflictState conflictState)
	{
		...
		float keyPropertyHeight = EditorGUI.GetPropertyHeight(keyProperty);
		float valuePropertyHeight = valueProperty != null ? GetDisplayedPropertyHeight(valueProperty) : 0f;
