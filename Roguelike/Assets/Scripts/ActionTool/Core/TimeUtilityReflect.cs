...
{
	public enum EStandardFrameRates
	{
		/// <summary>
	...
	public readonly struct FrameRate : IEquatable<FrameRate>
	{
		...
		public bool IsValid() => rate > TimeUtilityReflect.kTimeEpsilon;
		public bool Equals(FrameRate other) => Math.Abs(rate - other.rate) < TimeUtilityReflect.kFrameRateEpsilon;
		...
		public static FrameRate DoubleToFrameRate(double rate) => new FrameRate(Math.Ceiling(rate) - rate < TimeUtilityReflect.kFrameRateEpsilon ? rate : Math.Ceiling(rate) * 1000.0 / 1001.0);
	}

	/// <summary>
	/// Reflect of UnityEngine.Timeline.TimeUtility (package ver 1.75)
	/// </summary>
	public static class TimeUtilityReflect //: UnityEngine.Timeline.TimeUtility
	{
		...
		public static string TimeAsTimeCode(double timeValue, double frameRate, string format = "F2")
		{
			...
			string frames = (ToFrames(timeValue, frameRate) - ToFrames(intTime, frameRate)).ToString()
				.PadLeft(frameDigits, '0');
		...
		public static FrameRate ToFrameRate(EStandardFrameRates enumValue)
		{
			switch (enumValue)
			{
				case EStandardFrameRates.Fps23_97:
					return FrameRate.k_23_976Fps;
				case EStandardFrameRates.Fps24:
					return FrameRate.k_24Fps;
				case EStandardFrameRates.Fps25:
					return FrameRate.k_25Fps;
				case EStandardFrameRates.Fps29_97:
					return FrameRate.k_29_97Fps;
				case EStandardFrameRates.Fps30:
					return FrameRate.k_30Fps;
				case EStandardFrameRates.Fps50:
					return FrameRate.k_50Fps;
				case EStandardFrameRates.Fps59_94:
					return FrameRate.k_59_94Fps;
				case EStandardFrameRates.Fps60:
		...
		internal static bool ToStandardFrameRate(FrameRate rate, out EStandardFrameRates standard)
		{
			if (rate == FrameRate.k_23_976Fps)
				standard = EStandardFrameRates.Fps23_97;
			else if (rate == FrameRate.k_24Fps)
				standard = EStandardFrameRates.Fps24;
			else if (rate == FrameRate.k_25Fps)
				standard = EStandardFrameRates.Fps25;
			else if (rate == FrameRate.k_30Fps)
				standard = EStandardFrameRates.Fps30;
			else if (rate == FrameRate.k_29_97Fps)
				standard = EStandardFrameRates.Fps29_97;
			else if (rate == FrameRate.k_50Fps)
				standard = EStandardFrameRates.Fps50;
			else if (rate == FrameRate.k_59_94Fps)
				standard = EStandardFrameRates.Fps59_94;
			else if (rate == FrameRate.k_60Fps)
				standard = EStandardFrameRates.Fps60;
			else
			{
				standard = (EStandardFrameRates)Enum.GetValues(typeof(EStandardFrameRates)).Length;
				return false;
			}
		...
		public static float ToFrameRateValue(EStandardFrameRates enumValue)
		{
			return (float)ToFrameRate(enumValue).rate;
		}

		public static double FromTimeString(string timeString, double frameRate, double defaultValue)
		{
